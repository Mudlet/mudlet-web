/**
 * `QColor(QString)` for the forms Mudlet's colour arguments accept, answering
 * `[r, g, b]` (0..255) or null for a string QColor would call invalid.
 *
 * Deliberately not `CSS.supports('color', …)`: CSS takes far more than QColor
 * does (`rgb(…)`, `hsl(…)`, `currentcolor`), so a script that worked here would
 * be refused by Mudlet, and it reads `#AARRGGBB` as `#RRGGBBAA`. The forms are
 * Qt's own: `#RGB`, `#RRGGBB`, `#AARRGGBB`, `#RRRGGGBBB`, `#RRRRGGGGBBBB`, and
 * the SVG 1.0 colour keywords, case-insensitively. A half-written hex such as
 * `#ff` is invalid, as it is in Qt. Alpha is not returned: the callers here
 * (setSvgTint) build an opaque colour from the three channels.
 */
export function parseQColor(input: string): [number, number, number] | null {
    const s = input.trim();
    if (s.startsWith('#')) {
        const hex = s.slice(1);
        if (!/^[0-9a-f]+$/i.test(hex)) return null;
        const channels = (width: number, offset = 0): [number, number, number] => {
            const max = 16 ** width - 1;
            const at = (i: number) => Math.round(parseInt(hex.slice(offset + i * width, offset + (i + 1) * width), 16) * 255 / max);
            return [at(0), at(1), at(2)];
        };
        switch (hex.length) {
            case 3: return channels(1);
            case 6: return channels(2);
            case 8: return channels(2, 2); // #AARRGGBB — the alpha comes first
            case 9: return channels(3);
            case 12: return channels(4);
            default: return null;
        }
    }
...
const SVG_COLOR_NAMES: Record<string, number> = {
