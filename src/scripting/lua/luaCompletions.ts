...
const MUDLET_GLOBALS: Completion[] = [
    ...
    fn('clearVisitedLinks', '()',          'Forget which clickable links have been visited. mudlet tracks no visited-link state, so this is a no-op (kept for Mudlet script portability).'),
    ...
    fn('setMainWindowSize', '(width, height)', 'Sizes the main viewport — the rectangle getMainWindowSize reports. Mudlet resizes the application window here; a browser tab cannot resize itself, so the viewport inside it is what moves, and the two sizes agree with no window chrome between them.'),
    fn('setAppStyleSheet', '(qss [, tag]) → bool', 'Install an application-wide QSS sheet. An optional tag names it, so a later call with the same tag replaces that sheet instead of stacking a second one. Profile-local in Mudlet Web (Mudlet shares one QApplication sheet across open profiles) and removed when the profile closes.'),
    ...
    fn('disableHorizontalScrollBar', '([window])',      'Hide the horizontal scrollbar gutter. mudlet wraps long lines by default, so this is rarely needed; included for Mudlet parity.'),
    ...
    fn('alert',             '([seconds])',   'Flash for attention. Browsers can\'t flash the taskbar, so mudlet flashes the document title for `seconds` (default 10). No-op while the tab is already focused.'),
    ...
    fn('calcFontSize',      '(size[, family]) | (windowName) → w, h',
       'Measure the average character cell of the named window or of an explicit size/family combo. Returns pixel width, height — whole pixels, like Mudlet\'s QFontMetrics, so N * width is a real widget size. Use it to size a miniconsole to fit N cols × M rows; for the columns that actually fit a live window, prefer getColumnCount. Returns (nil, errMsg) for an unknown window or invalid size.'),
    ...
    fn('getMainConsoleWidth', '() → number', 'Pixel width of the main console text area — monospace cell width × (wrap columns + 1). Wrap columns come from setWindowWrap("main", n) (100 by default), else the live measured column count when wrapping is off in Settings.'),
    fn('setWindowWrap',   '(window, cols) → bool', 'Set the line-wrap width (in monospace columns) for a window (use "main" for the main output). Longer lines are stored and shown as several lines. Must be at least 1.'),
    fn('getWindowWrap',   '(window) → number', 'Get the line-wrap width (monospace columns) of a window ("main" for the main output). 100 for main and 99999999 for a miniconsole or user window unless set; -1 when the window does not exist.'),
    ...
    fn('highlightRoom',        '(id, r1, g1, b1, r2, g2, b2, radius [, a1, a2]) → bool',
       'Highlight a room with an overlay color, overriding its environment color. mudlet paints with color1 (alpha a1) — color2/radius round-trip via getRoomHighlights but are not visualized.'),
    ...
    fn('setSvgTint',          '(labelName, r, g, b | colorName) → true', 'Tint a label\'s SVG background: the colour replaces every opaque pixel. Takes a Mudlet colour name, an SVG colour name or "#rrggbb". The tint belongs to the label and survives a new image. (nil, errMsg) on a bad colour or missing label.'),
    fn('resetSvgTint',        '(labelName) → true',                   'Remove the tint set by setSvgTint, restoring the SVG\'s own colours.'),
    fn('setSvgRotation',      '(labelName, degrees) → true',          'Rotate a label\'s SVG background about its centre (positive = clockwise). Content outside the label is clipped.'),
    fn('resetSvgRotation',    '(labelName) → true',                   'Reset the SVG background rotation to 0 degrees.'),
    fn('setSvgShear',         '(labelName, shearX, shearY) → true',   'Shear (skew) a label\'s SVG background about its centre.'),
    fn('resetSvgShear',       '(labelName) → true',                   'Reset the SVG background shear to (0, 0).'),
    fn('resetSvgTransform',   '(labelName) → true',                   'Reset both the rotation and the shear of a label\'s SVG background; the tint stays.'),
    ...
    fn('getPausedSounds',     '([filter]) → {}',                     'Always returns an empty list — mudlet\'s Web Audio backend stops rather than pauses sound effects, so none ever sit paused. Kept for ported-script parity.'),
    fn('getPausedMusic',      '([filter]) → {}',                     'Always returns an empty list — mudlet stops rather than pauses music tracks. Kept for ported-script parity.'),
