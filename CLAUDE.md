...
> **Naming.** The project was renamed from *mudix* to *Mudlet Web*. **Browser storage followed the rename** — `mudlet_*` localStorage keys and IndexedDB databases, and the `.mudlet/` VFS directory. That was blocked for a long time because the keys *are* the user's data: renaming them alone orphans every existing profile. `src/storage/storageMigration.ts` is what unblocked it, moving an existing browser's data to the new names before anything reads them (localStorage synchronously at import, since `persist` hydrates then; IndexedDB behind every `openDb()`), and `.mudix/` is moved to `.mudlet/` lazily when a profile is opened. **Two things still deliberately keep the old name:** the `__mudix_*` Lua globals in `Bridge.lua` (name-paired with their JS call sites) and the `.mudix-*` CSS classes (a documented brand-styling API). Don't rename those opportunistically. Names that appear *inside a file format* are frozen too — a `mudlet-web-backup` and any `.zip` exported before the rename carry the old spellings, so `backupImport.ts` and the export sidecar readers accept both. The library entry points *did* follow the rename: the package is `@mudlet/mudlet-web` (npm), the app root export is `MudletWebApp` (`src/MudletWebApp.tsx`), and no `MudixApp` alias is kept.
